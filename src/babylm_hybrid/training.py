"""Bounded, auditable D/E baseline engine; no method interventions.
...
def _validate_protocol(p):
    ...
    if p['device'] == 'cuda' and (not math.isfinite(float(p.get('hourly_rate_usd', 0))) or p.get('hourly_rate_usd', 0) <= 0):
...
def train_run(protocol: dict, mode: str, output_dir: Path, resume_checkpoint: Path | None = None,
    ...
    def cost_time_reason():
        ...
        if p['scope'] != 'engineering_smoke' and float(p.get('stage_spent_usd', 0)) + estimated_paid_cost() >= float(p['paid_ceiling_usd']):
    ...
    try:
        ...
        summary = {'status': stop_reason, 'scope': p['scope'], 'mode': mode, 'protocol_sha256': phash,
                   ...
                   'cost_limitations': 'Hourly estimate, not an invoice; external idle time/storage and abrupt process death need launcher reconciliation.',
