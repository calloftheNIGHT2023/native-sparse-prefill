...
def main(a):
 ...
 protocol=json.loads((R/'provenance/word-suffix-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  with torch.no_grad():
   for j,item in enumerate(meta):
    ...
    seq=flat[offsets[j]:offsets[j+1]].astype(np.int64);assert len(seq)==32768 and seq[-len(suffix):].tolist()==suffix
    ids=torch.tensor(seq[None],device='cuda');torch.cuda.synchronize();pt=time.perf_counter()
    ...
    pred=dict(**item,prediction=guess,correct=guess==item['gold'],choice_logits=values,suffix_start=state['start'],diagnostic_forward_seconds=elapsed);predictions.append(pred)
    with (out/'task-predictions.jsonl').open('a') as f:f.write(json.dumps(pred)+'\n')
    if len(predictions)%16==0:event('prediction_progress',completed=len(predictions),maximum=64)
