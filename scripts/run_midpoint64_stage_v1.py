...
def main():
    pp=R/'provenance/midpoint64-protocol-v1.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/midpoint64-stage-v1';out.mkdir(exist_ok=False);state=R/'logs/midpoint64-queue-v1.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    save(state,dict(status='running',utc=start,optimizer_updates=0))
    try:
        for j in p['jobs']:
            if j['phase']=='midpoint':assert replayed=={0,32,48}
            remaining=p['maximum_seconds']-(time.monotonic()-tick);assert remaining>10
            cmd=[sys.executable,'-u',str(R/'scripts/eval_midpoint64_v1.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/midpoint64-evidence-v1.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('midpoint64-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
