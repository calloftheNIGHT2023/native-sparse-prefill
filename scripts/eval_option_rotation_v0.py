...
def main(a):
 ...
 protocol=json.loads((R/'provenance/option-rotation-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  meta=json.loads((R/'data/option-rotation-v0/tasks.json').read_text());arrays=np.load(R/'data/option-rotation-v0/tasks.npz');flat=arrays['input_ids'];offsets=arrays['offsets']
  reference={}
  if True:
   assert sha(R/job['reference'])==job['reference_sha256']
   reference={(x['item_id'],x['variant']):x for x in json.loads((R/job['reference']).read_text())['predictions']}
  predictions=[];max_logit_error=0.;labels=torch.tensor(cfg['label_token_ids'],device='cuda')
  with torch.no_grad():
   for j,item in enumerate(meta):
    if item['variant'] not in protocol['variants'] or (job['item_ids'] is not None and item['item_id'] not in job['item_ids']):continue
    ...
    pred.update(rotation=item['rotation'],original_gold=item['original_gold'],semantic_prediction=(guess+item['rotation'])%4,original_replay=item['original_replay'])
    if item['original_replay']:
     ref=reference[(item['item_id'],'long32768')];delta=max(abs(x-y) for x,y in zip(values,ref['choice_logits']));max_logit_error=max(max_logit_error,delta)
     assert delta<=protocol['replay_logit_max_abs_error'] and guess==ref['prediction']
    predictions.append(pred)
    if len(predictions)%16==0:event('prediction_progress',completed=len(predictions),expected=512)
    with (out/'task-predictions.jsonl').open('a') as f:f.write(json.dumps(pred)+'\n')
  assert len(predictions)==512 and sum(x['original_replay'] for x in predictions)==64
  result=dict(status='complete',job=job,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,step=step,optimizer_updates=0,task_predictions=len(predictions),calibration_values=vals,calibration_max_abs_error=error,replay_logit_max_abs_error=max_logit_error,checkpoint_sha256=sha(checkpoint),eval_source_sha256=sha(Path(__file__)),predictions=predictions)
 except Exception:
