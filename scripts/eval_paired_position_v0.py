...
def main(a):
 ...
 protocol=json.loads((R/'provenance/paired-position-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  meta=json.loads((R/'data/paired-position-v0/tasks.json').read_text());arrays=np.load(R/'data/paired-position-v0/tasks.npz');flat=arrays['input_ids'];offsets=arrays['offsets']
  reference={}
  if True:
