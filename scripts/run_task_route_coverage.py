...
def main():
 protocol_path=R/'provenance/task-quality-route-coverage-protocol.json';protocol=read(protocol_path)
 for name,h in protocol['sources'].items():assert hashlib.sha256((R/name).read_bytes()).hexdigest()==h,name
 out=R/'results/task-route-coverage-stage-v0';out.mkdir(exist_ok=False)
 ...
 for seed,parent in [(2026091560,R/'parents/k16-seed0'),(2026091561,R/'results/amp-recovery-stage-v0/repeat-k16')]:
  name=f'seed{seed}';jobs.append((name,[sys.executable,'-u',str(R/'scripts/eval_task_route_coverage.py'),'--run-dir',str(parent),'--output',str(out/name)],protocol['per_job_max_seconds']))
 try:
  ...
  gate=dict(not_applicable=True,reason='baseline metadata audit only');status='complete'
 ...
 archive=R/'exports/task-route-coverage-evidence-v0.tar.gz';assert not archive.exists();entries=[]
 with tarfile.open(archive,'w:gz') as tar:
  ...
  raw=json.dumps(dict(files=entries,completed_job_directories=[f'results/task-route-coverage-stage-v0/{r["name"]}' for r in runs if r['name']!='preflight' and r['returncode']==0]),indent=2).encode()
