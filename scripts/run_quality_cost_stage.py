...
OUT=ROOT/'results/flashmoba-quality-cost-controller-v0';OUT.mkdir(parents=True,exist_ok=False)
...
try:
    for si,seed in enumerate(cfg['initialization_seeds']):
        ...
        for cond in order:
            name=f"{cond['name']}-s{seed}";folder=ROOT/'results'/f'flashmoba-quality-cost-{name}-v0'
