...
R=Path(__file__).resolve().parents[1];STAGE='lambada-natural-retry'
...
def main():
    ...
    try:
        while time.monotonic()-start<2400:
            code="from pathlib import Path\nimport json\nr=Path("+repr(root)+")\nf=r/'logs/lambada-natural-retry-queue-v0.json'\nprint(json.dumps(dict(queue=json.loads(f.read_text()) if f.exists() else None,archive_ready=(r/'exports/lambada-natural-retry-evidence-v0.tar.json').exists())))\n"
            try:
        ...
        with (R/f'logs/{STAGE}-v0-audit.log').open('w',encoding='utf-8') as f:subprocess.run([sys.executable,str(R/'scripts/report_lambada_natural_retry_v0.py')],cwd=R,stdout=f,stderr=subprocess.STDOUT,check=True,timeout=300)
        result=json.loads((R/f'results/{STAGE}-audit-v0/result.json').read_text(encoding='utf-8'));assert result['status']=='verified'
        c=json.loads((R/'logs/control-state.json').read_text(encoding='utf-8'));already=c.get('lambada_natural_retry',{}).get('status')=='complete_verified'
        if not already:
            assert c['cumulative_task_predictions']==36607
            c['cumulative_task_predictions']+=result['task_predictions']
        c.update(status='lambada_natural_retry_v0_complete_verified',updated_utc=utc(),active_evaluation_jobs=0,next_action='Inspect natural full-vocabulary word prediction and base context gate alongside matched7.6-7.7%training artifact cost. If base gate failed, preserve and do not call it sparse failure; choose a documented suitable natural task without outcome-guided revisions. If retention passed, consolidate evidence and screen contribution overlap before independent-seed/generalization expansion; known QK-Restore and LongLoRA are not original contributions. Current0.5B LoRA evidence not automatically sufficient for paper. Keep no inference-speed requirement.')
        c['lambada_natural_retry']=dict(status='complete_verified',report='docs/lambada-natural-retry-results-2026-09-17.md',task_predictions=result['task_predictions'],calibration_nll_forwards=result['calibration_nll_forwards'],base_gate=result['base_gate'],natural_accuracy_screen_pass=result['natural_accuracy_screen_pass'],contrasts=result['contrasts'],archive=result['archive']);save(R/'logs/control-state.json',c)
        ...
        for n in ['STATE.md','TIMELINE.md','logs/control-state.json','docs/lambada-natural-retry-results-2026-09-17.md',f'logs/{STAGE}-v0-cost-estimate.json']:subprocess.run(scp+[str(R/n),host+':'+root+'/'+n],check=True,timeout=45)
        save(state,dict(status='complete_verified',utc=utc(),report='docs/lambada-natural-retry-results-2026-09-17.md'))
    except Exception:
