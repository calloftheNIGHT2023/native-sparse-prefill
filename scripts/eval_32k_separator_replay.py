"""Read-only separator-consistency replay; original training identity remains unchanged."""
import os
...
def main(a):
 ...
 try:
  ...
  repair=json.loads((R/'provenance/32k-separator-replay-protocol.json').read_text())
  assert sha(Path(__file__))==repair['evaluator_sha256']
  ...
  if a.phase=='preflight':
   ...
   f=R/'data/32k-separator-replay-v0/report.npz';assert sha(f)==repair['data_sha256']
   report=np.load(f)['report'];assert report.shape==(9,32769);evaluate('report',report)
   save(out/'separator-replay.json',dict(protocol_sha256=sha(R/'provenance/32k-separator-replay-protocol.json'),data_sha256=sha(f),evaluator_sha256=sha(Path(__file__)),checkpoint_sha256=sha(a.resume),source_training_identity_unchanged=True))
