...
OUT=ROOT/'results/flashmoba-mixed-cost-analysis-v0';OUT.mkdir(parents=True,exist_ok=False)
cfg=json.loads((ROOT/'configs/flashmoba-mixed-cost-v0.json').read_text())
controller=json.loads((ROOT/'results/flashmoba-mixed-cost-controller-v0/result.json').read_text())
rows=[];gates=[];failed=[];aggregate=[]
for precision,length in cfg['configurations']:
    path=ROOT/'results'/f'flashmoba-mixed-cost-{precision}-{length}-v0/result.json'
    if not path.exists():
