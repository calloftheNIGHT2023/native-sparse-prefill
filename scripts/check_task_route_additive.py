...
from task_route_additive import routed_attention,replace_question_blocks,metadata_from_ids
...
for n in [512,2048,4096]:
 ...
 if n==4096:
  y,forced=routed_attention(q,k,v,.125,'add_target',start=4000,forced=[3,4],target=[3,4],return_ids=True,sham_candidates=[i for i in range(2,30) if i not in [3,4]]);assert torch.isfinite(y).all()
  assert torch.equal(y[:4000],native[:4000])
  assert (forced[-1]>=0).sum().item()==14*18 and all(i in forced[-1,0].tolist() for i in [3,4,31])
  sham,shamids=routed_attention(q,k,v,.125,'add_sham',start=4000,forced=[3,4],target=[3,4],return_ids=True,sham_candidates=[i for i in range(2,30) if i not in [3,4]])
  assert torch.equal((forced>=0).sum(-1),(shamids>=0).sum(-1)) and torch.equal(sham[:4000],native[:4000])
  refs=[]
...
out=Path('results/task-route-additive-preflight-v0');out.mkdir(parents=True,exist_ok=False)
