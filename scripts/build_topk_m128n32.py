...
def main():
 started=datetime.now(timezone.utc).isoformat();tick=time.monotonic();src=R/'experiments/topk-m128n32-v0';m=json.loads((src/'manifest.json').read_text())
 for n,h in m['files'].items():assert sha(src/n)==h,n
 deps=R/'experiments/topk-m64-v0/deps';deps.mkdir(exist_ok=True);commit=m['cutlass_commit'];archive=deps/f'cutlass-{commit}.tar.gz';include=deps/f'cutlass-{commit}/include'
 ...
 ns='-DFLASH_TOPK_NAMESPACE=nsp_topk_m128n32_v0'
 module=load(name='nsp_topk_m128n32_v0',sources=[str(src/'api.cpp'),str(src/'kernel.cu')],extra_include_paths=[str(src),str(include)],extra_cflags=['-O2',ns],extra_cuda_cflags=['-O2',ns,'--expt-relaxed-constexpr','--expt-extended-lambda','-U__CUDA_NO_HALF_OPERATORS__','-U__CUDA_NO_HALF_CONVERSIONS__','-U__CUDA_NO_BFLOAT16_CONVERSIONS__','-U__CUDA_NO_HALF2_OPERATORS__'],build_directory=str(build),verbose=True)
