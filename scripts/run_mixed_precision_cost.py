...
def main(a):
    out = ROOT/'results'/f'flashmoba-mixed-cost-{a.precision}-{a.length}-v0'
    ...
    try:
        ...
        stream=np.concatenate([train[0,:-1],train[1]])
        ...
        for name,x,loss,grad,ref,rloss,rg,tol in [
            ('eager_vs_dense',dense,dloss,dg,native,nloss,ng,.02),
            ('dense_vs_allblocks',sparse,sloss,sg,dense,dloss,dg,.01)]:
            metrics=dict(name=name,logits_relative_l2=float((x-ref).norm()/ref.norm()),
                         nll_abs_difference=abs(loss-rloss),gradient_relative_l2=float((grad-rg).norm()/rg.norm()),
                         logits_tolerance=tol,nll_tolerance=.05,gradient_tolerance=.10)
            metrics['passed']=metrics['logits_relative_l2']<tol and metrics['nll_abs_difference']<.05 and metrics['gradient_relative_l2']<.10
            gates.append(metrics);save(out/'gates.json',gates)
            assert metrics['passed'],metrics
        del native,ng,dense,dg,sparse,sg,x,grad,ref,rg
        ...
        save(out/'manifest.json',dict(started_utc=started,precision=a.precision,length=a.length,
            ...
            config_sha256=sha(ROOT/'configs/flashmoba-mixed-cost-v0.json'),initial_adapters_sha256=initial,
        ...
        for rnd,order in enumerate([[0,4,16],[16,4,0]],1):
            for k in order:
                state['k']=k
                for mode in ['prefill','forward_backward']:
        ...
        result=dict(status='complete',started_utc=started,finished_utc=utc(),wall_seconds=time.perf_counter()-wall_start,
                    precision=a.precision,length=a.length,rows=rows,gates=gates,scientific_optimizer_updates=0,
...
if __name__=='__main__':
    ...
    p.add_argument('--length',type=int,choices=[8192,16384],required=True);main(p.parse_args())
