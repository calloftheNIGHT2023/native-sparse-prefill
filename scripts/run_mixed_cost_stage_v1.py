...
OUT=ROOT/'results/flashmoba-mixed-cost-controller-v1';OUT.mkdir(parents=True,exist_ok=False)
...
for precision,length in cfg['configurations']:
    ...
    cmd=['/opt/native-sparse-flashmoba-env-v2/bin/python','-u','scripts/run_mixed_precision_cost_v1.py','--precision',precision,'--length',str(length)]
    ...
    if code:break
result=dict(status='complete' if len(rows)==2 and all(x['returncode']==0 for x in rows) else 'stopped_on_failure',
