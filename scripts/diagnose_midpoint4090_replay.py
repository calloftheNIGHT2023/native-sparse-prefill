...
def main(a):
 ...
 protocol=json.loads((R/'provenance/midpoint64-protocol-v1.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 assert job['phase']=='replay'
 ...
 try:
  ...
  meta=json.loads((data/'tasks.json').read_text());arrays=np.load(data/'tasks.npz');flat=arrays['input_ids'];offsets=arrays['offsets']
  reference={}
  if job['phase']=='replay':
  ...
  with torch.no_grad():
   for j,item in enumerate(meta):
    ...
    if reference:
     ref=reference[(item['item_id'],item['variant'])];delta=max(abs(x-y) for x,y in zip(values,ref['choice_logits']));max_logit_error=max(max_logit_error,delta)
     pred.update(reference_choice_logits=ref['choice_logits'],reference_prediction=ref['prediction'],logit_max_abs_error=delta,replay_passed=delta<=protocol['replay_logit_max_abs_error'] and guess==ref['prediction'])
    predictions.append(pred)
    with (out/'task-predictions.jsonl').open('a') as f:f.write(json.dumps(pred)+'\n')
  assert len(predictions)==(8 if job['phase']=='replay' else 128)
