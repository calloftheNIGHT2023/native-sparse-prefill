"""One bounded, fresh sparse LR intervention; frozen training engine unchanged.
...
def derive_child(base, master):
    require(base["indexer_learning_rate"] == 1e-3 and master["indexer_learning_rate"] == 1e-4,
            "This probe freezes indexer peak LR 1e-3 -> 1e-4 only")
    ...
    child["indexer_learning_rate"] = master["indexer_learning_rate"]
    child.update(copy.deepcopy(overrides))
    # Retained original notes/metadata are explicitly historical provenance;
    # they do not describe this probe's changed LR, hardware, or stopping point.
    child["probe_execution"] = {"kind": "fresh_indexer_lr_prefix", "base_protocol": master["base_protocol"],
        ...
        "scientific_changes": {"indexer_learning_rate": {"before": 1e-3, "after": 1e-4}},
    ...
    require({k for k in set(base) | set(child) if base.get(k) != child.get(k)}
            <= {"indexer_learning_rate", "probe_execution"} | set(overrides),
...
def validate(master):
    ...
    require(pins.get("scripts/run_babylm_indexer_lr_probe_v0.py") == sha(__file__), "Probe launcher is not pinned")
    ...
    return {"kind": "validation_only_no_model_calls", "child_protocol": child,
            ...
            "scientific_changes": {"indexer_learning_rate": {"before": 0.001, "after": 0.0001}}}
...
def audit_prefix(run_dir, plan):
    ...
    return {"status": "complete_prefix_audited", "scope": "Fresh sparse 500-update LR intervention; not a complete epoch",
