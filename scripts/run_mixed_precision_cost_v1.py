...
def main(a):
    out = ROOT/'results'/f'flashmoba-mixed-cost-{a.precision}-{a.length}-v1'
    out.mkdir(parents=True, exist_ok=False)
    started=utc();wall_start=time.perf_counter();rows=[];gates=[]
    (out/'source.py').write_bytes(Path(__file__).read_bytes())
    cfg=json.loads((ROOT/'configs/flashmoba-mixed-cost-v1.json').read_text())
    ...
    try:
        ...
        save(out/'manifest.json',dict(started_utc=started,precision=a.precision,length=a.length,
            ...
            config_sha256=sha(ROOT/'configs/flashmoba-mixed-cost-v1.json'),initial_adapters_sha256=initial,
