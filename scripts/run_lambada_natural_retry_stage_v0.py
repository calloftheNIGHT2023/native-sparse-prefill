...
def main():
    pp=R/'provenance/lambada-natural-retry-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/lambada-natural-retry-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/lambada-natural-retry-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_lambada_natural_retry_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    if status=='complete':assert count in [1024,p['expected_task_predictions']]
    gradient_count=0
    ...
    archive=R/'exports/lambada-natural-retry-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('lambada-natural-retry-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
