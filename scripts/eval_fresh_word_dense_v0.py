...
def main(a):
 ...
 protocol=json.loads((R/'provenance/fresh-word-dense-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  state['k']=0
  event('common_dense_evaluation',training_k=a.k,evaluation_k=0)
  data=R/'data/fresh-word-dense-v0';meta=json.loads((data/'tasks.json').read_text());arrays=np.load(data/'tasks.npz');flat=arrays['input_ids'];offsets=arrays['offsets']
  ...
  with torch.no_grad():
   for j,item in enumerate(meta):
    if item['variant']=='long32768' and short_gate is None:
     short=[x for x in predictions if x['variant']=='short'];assert len(short)==4
     short_gate=dict(correct=sum(x['correct'] for x in short),total=4,passed=all(x['correct'] for x in short));event('short_gate',**short_gate)
     if job['phase']=='base_gate' and not short_gate['passed']:break
    ...
    pred=dict(**item,prediction=guess,choice_logits=values)
    ...
    if len(predictions)%16==0:event('prediction_progress',completed=len(predictions),maximum=133)
  long=[x for x in predictions if x['variant']=='long32768']
  if long:
   assert len(long)==128;correct=sum(x['correct'] for x in long);long_gate=dict(correct=correct,total=128,passed=correct>=protocol['base_long_min_correct'])
  assert len(predictions) in [5,133] and (job['phase']=='base_gate' or len(predictions)==133)
  result=dict(status='complete',job=job,evaluation_k=0,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,step=step,optimizer_updates=0,task_predictions=len(predictions),short_gate=short_gate,long_gate=long_gate,calibration_values=vals,calibration_max_abs_error=error,checkpoint_sha256=sha(checkpoint),eval_source_sha256=sha(Path(__file__)),predictions=predictions)
