...
R=Path(__file__).resolve().parents[1];out=R/'results/resume-environment-20260917-v1';env=Path('/opt/nsp-resume-env-v2')
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def main():
 out.mkdir(exist_ok=False);started=datetime.now(timezone.utc).isoformat();tick=time.monotonic();state=R/'logs/resume-environment-20260917-v1.json'
