...
out=R/'results/length-density-cost-stage-v0';out.mkdir(exist_ok=False)
start=utc();tick=time.monotonic();error=None
cmd=[sys.executable,'-u',str(R/'scripts/benchmark_length_density_cost.py'),'--run-dir',str(R/'parents/dense-seed0'),'--output',str(out/'benchmark')]
with (out/'benchmark.log').open('w') as f:
 ...
 except subprocess.TimeoutExpired:p.kill();p.wait();code=-9;error='360-second time cap'
...
files += [R/'scripts/benchmark_length_density_cost.py',Path(__file__),R/'provenance/length-density-cost-protocol.json']
a=R/'exports/length-density-cost-evidence-v0.tar.gz';assert not a.exists();rows=[]
with tarfile.open(a,'w:gz') as t:
 ...
 raw=json.dumps(dict(files=rows,completed_job_directories=['results/length-density-cost-stage-v0/benchmark'] if code==0 else []),indent=2).encode();m=tarfile.TarInfo('amp-recovery-results-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
