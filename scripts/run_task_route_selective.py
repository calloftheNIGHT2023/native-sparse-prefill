...
def main():
 protocol_path=R/'provenance/task-quality-route-selective-protocol.json';protocol=read(protocol_path)
 for name,h in protocol['sources'].items():assert hashlib.sha256((R/name).read_bytes()).hexdigest()==h,name
 out=R/'results/task-route-selective-stage-v0';out.mkdir(exist_ok=False)
 start=utc();tick=time.monotonic();runs=[];gate=None;error=None
 jobs=[('preflight',[sys.executable,'-u',str(R/'scripts/check_task_route_selective.py')],120)]
 for seed,parent in [(2026091560,R/'parents/k16-seed0'),(2026091561,R/'results/amp-recovery-stage-v0/repeat-k16')]:
  name=f'seed{seed}';jobs.append((name,[sys.executable,'-u',str(R/'scripts/eval_task_route_selective.py'),'--run-dir',str(parent),'--output',str(out/name)],protocol['per_job_max_seconds']))
 try:
  ...
  delta={c:[s['selected_target']-s[c] for s in scores] for c in ['baseline','selected_sham','random_target']}
  thresholds=protocol['gate']
  passed=all(min(delta[c])>=thresholds[c]['each_seed'] and sum(delta[c])/2>=thresholds[c]['mean'] for c in delta)
 ...
 files=[p for sub in [out,R/'results/task-route-selective-preflight-v0'] for p in sub.rglob('*') if p.is_file()]
 files += [R/name for name in protocol['sources']] + [protocol_path]
 files += [R/'provenance/task-quality-route-chain-protocol.json',R/'docs/task-route-selective-protocol-2026-09-15.md']
 archive=R/'exports/task-route-selective-evidence-v0.tar.gz';assert not archive.exists();entries=[]
 with tarfile.open(archive,'w:gz') as tar:
  ...
  raw=json.dumps(dict(files=entries,completed_job_directories=[f'results/task-route-selective-stage-v0/{r["name"]}' for r in runs if r['name']!='preflight' and r['returncode']==0]),indent=2).encode()
