...
def main(a):
 ...
 protocol=json.loads((R/'provenance/midpoint64-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  trained=json.loads((R/job['training_result']).read_text());assert trained['environment']==environment
