...
def main(a):
 ...
 protocol=json.loads((R/'provenance/word-counterfactual-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  with torch.no_grad():
   for j,item in enumerate(meta):
    if item['variant']=='long32768' and short_gate is None:
     ...
     if not short_gate['passed']:break
  ...
  if short_gate['passed']:long_gate=gate('long32768',protocol['long_gate']);event('long_ability_gate',**long_gate)
  assert len(predictions)==(192 if short_gate['passed'] else 128)
