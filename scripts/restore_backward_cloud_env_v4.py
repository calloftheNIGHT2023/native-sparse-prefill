...
OUT=ROOT/'results/flashmoba-backward-environment-v4';OUT.mkdir(parents=True,exist_ok=False)
...
try:
    ...
    for name in ['flash_moba.egg-info','build']:
        p=CAND/name
        if p.exists():
            assert p.resolve().parent==CAND.resolve() and not p.is_symlink()
            shutil.move(str(p),str(OUT/('migrated-'+name)))
            event('preserved_stale_generated_files',path=name)
