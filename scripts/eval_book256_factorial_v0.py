...
def main(a):
 ...
 protocol=json.loads((R/'provenance/book256-factorial-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  wm=json.loads((R/'data/fresh-book256-v0/tasks.json').read_text());wa=np.load(R/'data/fresh-book256-v0/tasks.npz');wf=wa['input_ids'];wo=wa['offsets'];labels=torch.tensor(protocol['answer_token_ids'],device='cuda');word_preds=[]
  with torch.no_grad():
