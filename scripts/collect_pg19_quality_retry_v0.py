...
def main():
    ...
    try:
        while time.monotonic()-start<2700:
            code="from pathlib import Path\nimport json\nr=Path("+repr(root)+")\nf=r/'logs/pg19-quality-retry-queue-v0.json'\nprint(json.dumps(dict(queue=json.loads(f.read_text()) if f.exists() else None,archive_ready=(r/'exports/pg19-quality-retry-evidence-v0.tar.json').exists())))\n"
            try:
        ...
        with (R/f'logs/{STAGE}-v0-audit.log').open('w',encoding='utf-8') as f:
            subprocess.run([sys.executable,str(R/'scripts/report_pg19_quality_retry_v0.py')],cwd=R,stdout=f,stderr=subprocess.STDOUT,check=True,timeout=300)
        result=json.loads((R/f'results/{STAGE}-audit-v0/result.json').read_text(encoding='utf-8'));assert result['status']=='verified'
        c=json.loads((R/'logs/control-state.json').read_text(encoding='utf-8'));already=c.get('pg19_quality_retry',{}).get('status')=='complete_verified'
        assert c['cumulative_task_predictions']==34519
        c.update(status='pg19_quality_retry_v0_complete_verified',updated_utc=utc(),active_evaluation_jobs=0,next_action='Review external16-book full/tail quality and all symmetric restoration contrasts. If pass, preserve external evidence and design matched actual complete128-step sparse/dense training-cost confirmation with explicit budget, plus natural task evaluation; never reuse old-card speed as current GPU saving. If failed, preserve counterevidence and choose a distinct falsifiable hypothesis before more training. Existing QK-Restore remains prior art; not an original method or paper-ready evidence.')
        c['pg19_quality_retry']=dict(status='complete_verified',report='docs/pg19-quality-retry-results-2026-09-17.md',nll_forwards=result['nll_forwards'],task_predictions=0,external_sample_joint_pass=result['external_sample_joint_pass'],contrasts=result['contrasts'],archive=result['archive']);save(R/'logs/control-state.json',c)
        ...
        for n in ['STATE.md','TIMELINE.md','logs/control-state.json','docs/pg19-quality-retry-results-2026-09-17.md',f'logs/{STAGE}-v0-cost-estimate.json']:
            subprocess.run(scp+[str(R/n),host+':'+root+'/'+n],check=True,timeout=45)
        save(state,dict(status='complete_verified',utc=utc(),report='docs/pg19-quality-retry-results-2026-09-17.md'))
    except Exception:
