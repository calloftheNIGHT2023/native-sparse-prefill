...
def main(a):
 cfg=json.loads((R/'data/flashmoba-amp-recovery-v0/config.json').read_text());out=a.output;out.mkdir(parents=True,exist_ok=False)
 ...
 def run(name,phase,k,seed,lr,resume=None,selection=None):
  ...
  dest=out/name;cmd=[sys.executable,'-u',str(R/'scripts/run_amp_recovery.py'),'--phase',phase,'--output',str(dest),'--k',str(k),'--seed',str(seed),'--lr',str(lr)]
 ...
 (out/'result.json').write_text(json.dumps(dict(status=status,finished_utc=utc(),seconds=time.monotonic()-start,runs=runs,preflight_only=a.preflight_only),indent=2)+'\n')
