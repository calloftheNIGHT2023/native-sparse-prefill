...
def main(a):
 ...
 protocol=json.loads((R/'provenance/qk-restore-baseline-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  state['k']=0
  event('attention_operator_swap',training_k=a.k,evaluation_k=state['k'],checkpoint_step=step)
  ...
  report=np.load(R/'data/32k-expanded-training-v0/report.npz')['report'];report_values=[nll(w) for w in report];assert len(report_values)==9 and all(math.isfinite(x) for x in report_values)
  save(out/'report-nll.json',report_values)
