...
def main():
    pp=R/'provenance/vo-training-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/vo-training-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/vo-training-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/train_vo_only_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/vo-training-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('vo-training-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
