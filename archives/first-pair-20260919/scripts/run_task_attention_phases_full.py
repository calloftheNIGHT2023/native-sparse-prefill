...
def main():
 protocol_path=R/'provenance/task-attention-phases-full-protocol.json';protocol=read(protocol_path)
 for name,h in protocol['sources'].items():assert hashlib.sha256((R/name).read_bytes()).hexdigest()==h,name
 out=R/'results/task-attention-phases-full-stage-v0';out.mkdir(exist_ok=False)
 ...
 for seed,parent in [(2026091560,R/'parents/k16-seed0'),(2026091561,R/'results/amp-recovery-stage-v0/repeat-k16')]:
  name=f'seed{seed}';jobs.append((name,[sys.executable,'-u',str(R/'scripts/eval_task_attention_phases_full.py'),'--run-dir',str(parent),'--output',str(out/name)],protocol['per_job_max_seconds']))
 try:
 ...
 files += [R/'provenance/task-quality-route-chain-protocol.json',R/'docs/task-attention-phases-full-protocol-2026-09-16.md']
 archive=R/'exports/task-attention-phases-full-evidence-v0.tar.gz';assert not archive.exists();entries=[]
 with tarfile.open(archive,'w:gz') as tar:
  ...
  raw=json.dumps(dict(files=entries,completed_job_directories=[f'results/task-attention-phases-full-stage-v0/{r["name"]}' for r in runs if r['name']!='preflight' and r['returncode']==0]),indent=2).encode()
