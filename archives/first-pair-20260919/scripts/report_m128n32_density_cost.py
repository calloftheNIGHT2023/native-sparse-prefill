...
def main():
 a=R/'exports/m128n32-density-cost-evidence-v0.tar.gz';proof=load(a.with_suffix('.json'));assert sha(a)==proof['sha256'] and a.stat().st_size==proof['bytes']
 dest=R/'results/cloud-m128n32-density-cost-evidence-v0';dest.mkdir(exist_ok=True)
 with tarfile.open(a) as t:
  entries=json.loads(t.extractfile('m128n32-density-cost-manifest.json').read())['files'];names=[x.name for x in t.getmembers()];assert len(names)==len(set(names)) and set(names)=={x['path'] for x in entries}|{'m128n32-density-cost-manifest.json'} and len(entries)==proof['files']
  for e in entries:
 ...
 p=load(dest/'provenance/m128n32-density-cost-protocol.json');psha=sha(dest/'provenance/m128n32-density-cost-protocol.json');assert psha==sha(R/'provenance/m128n32-density-cost-protocol.json')
 ...
 stage=dest/'results/m128n32-density-cost-stage-v0';control=load(stage/'result.json');rows=[];count=0;order=np.random.default_rng(2026091662).permutation(32).tolist();paired={}
 for j in control['jobs']:
  ...
  assert m['implementation']['sha256']==p['candidate_module_sha256']
  ...
  assert sha(d/'diagnostic-final.pt')==m['diagnostic_checkpoint_sha256'] and sha(d/'source.py')==p['source_sha256']['scripts/benchmark_m128n32_density_cost.py']
 ...
 save(R/'results/m128n32-density-cost-audit-v0/result.json',result);save(dest/'LOCAL-VERIFICATION.json',dict(status='verified',archive=proof,checkpoints=count))
 lines=['# 修复资源限制后的K32/K48/K64完整更新速度筛选','',f"实验状态：{control['status']}。只做诊断，不把六步诊断更新当成质量训练结果。每个测量单元从同一密集64步断点恢复全部状态；两步预热、四步测量，包含前反向、裁剪、AdamW与scheduler。",'', '|轮次|K（0为密集）|测量中位秒/步|峰值显存GiB|数学检查|','|---|---:|---:|---:|---|']
 ...
 (R/'docs/m128n32-density-cost-results-2026-09-16.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
