...
OUT=ROOT/'results/flashmoba-mixed-cost-analysis-v1';OUT.mkdir(parents=True,exist_ok=False)
cfg=json.loads((ROOT/'configs/flashmoba-mixed-cost-v1.json').read_text())
controller=json.loads((ROOT/'results/flashmoba-mixed-cost-controller-v1/result.json').read_text())
rows=[];gates=[];failed=[];aggregate=[]
for precision,length in cfg['configurations']:
    path=ROOT/'results'/f'flashmoba-mixed-cost-{precision}-{length}-v1/result.json'
    if not path.exists():
    ...
    assert len(result['rows'])==12 and all(g['passed'] for g in result['gates'])
