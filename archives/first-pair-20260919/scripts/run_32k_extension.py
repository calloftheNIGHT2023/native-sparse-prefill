...
def main(a):
 ...
 try:
  protocol_path=R/'provenance/32k-extension-protocol.json'
  protocol=json.loads(protocol_path.read_text())
  assert sha(Path(__file__))==protocol['source_sha256']['scripts/run_32k_extension.py']
  ...
  if a.phase=='train':
   ...
   identity=job['resume_identity']
   step,extra=restore_checkpoint(a.resume,params,optimizer,scheduler,identity)
   assert step==128
   rows=extra['rows'];evaluations=extra['evaluations']
   assert len(rows)==128 and abs(sum(x['seconds'] for x in rows)-job['parent_training_seconds'])<1e-8
   original_cal=[e for e in evaluations if e['split']=='calibration' and e['step']==128][0]
   ...
   assert abs(optimizer.param_groups[0]['lr']-.0001)<1e-12 and scheduler.last_epoch==128
   budget=job['dense128_training_seconds'];crossed=False;compliant_step=128;compliant_seconds=sum(x['seconds'] for x in rows)
   event('resumed',step=128,parent_sha256=sha(a.resume),dense128_budget_seconds=budget)
