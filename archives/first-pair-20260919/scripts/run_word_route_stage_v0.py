...
def main():
    pp=R/'provenance/word-route-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/word-route-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/word-route-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    save(state,dict(status='running',utc=start,optimizer_updates=0))
    try:
        for j in p['jobs']:
            if j['condition']!='baseline':assert replayed==set(p['seeds'])
            remaining=p['maximum_seconds']-(time.monotonic()-tick);assert remaining>10
            cmd=[sys.executable,'-u',str(R/'scripts/eval_word_route_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
            ...
            if j['condition']=='baseline':replayed.add(j['seed'])
    ...
    archive=R/'exports/word-route-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('word-route-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
