...
import json,subprocess,sys,time,traceback,hashlib,tarfile,io
...
def main():
 out=R/'results/topk-m64-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/topk-m64-queue-v0.json';jobs=[];tick=time.monotonic();started=utc()
 ...
 files += [R/'scripts'/n for n in ['prepare_topk_m64.py','build_topk_m64.py','topk_m64_adapter.py','check_topk_m64.py','run_topk_m64_stage.py','density_tradeoff_math_gate.py','run_flashmoba_realtext_precision.py']]
 a=R/'exports/topk-m64-evidence-v0.tar.gz';assert not a.exists();entries=[]
