...
def source_identity():
 return {n:sha(R/'scripts'/n) for n in ['run_gentle32k.py','amp_recovery_state.py','chunked_lm_loss.py','run_flashmoba_realtext_precision.py']}
def main(a):
 import run_flashmoba_realtext_precision as base
 from topk_m64_adapter import enable
 implementation=enable(base)
 data=R/'data/32k-expanded-training-v0';cfg=json.loads((R/'data/gentle32k-v0/config.json').read_text())
 protocol=json.loads((R/'provenance/gentle32k-protocol.json').read_text())
 ...
 try:
  ...
  identity=dict(k=a.k,seed=a.seed,lr=a.lr,config_sha256=sha(R/'data/gentle32k-v0/config.json'),sources=source_identity(),extension_sha256=cfg['extension_sha256'])
  ...
  def update(i):
   ...
   loss=backward(train[order[i%76]]);norm=torch.nn.utils.clip_grad_norm_(list(params.values()),cfg['gradient_clip'],error_if_nonfinite=True)
   optimizer.step();scheduler.step();local_updates+=1;torch.cuda.synchronize()
   row=dict(step=i+1,window_index=order[i%76],train_nll=float(loss),gradient_norm=float(norm),lr=lr,seconds=time.perf_counter()-st,utc=utc())
  ...
  gates=[]
  if a.phase=='preflight':
   # Re-run original arithmetic gates plus genuinely sparse routing.
   ...
   ids,_=inputs(train[0]);altered=ids.clone();altered[:,16384:]=torch.roll(altered[:,16384:],17,1)
   ...
   step,extra=restore_checkpoint(a.resume,params,optimizer,scheduler,identity);assert step in [0,cfg['steps']]
   calibration_replay=[nll(w) for w in cal]
   original_cal=[e for e in extra['evaluations'] if e['step']==step and e['split']=='calibration'];assert len(original_cal)==1
   replay_error=max(abs(x-y) for x,y in zip(calibration_replay,original_cal[0]['values']));assert replay_error<=1e-6
   save(out/'replay-verification.json',dict(step=step,max_abs_error=replay_error,checkpoint_sha256=sha(a.resume)))
   assert sha(data/'report.npz')==cfg['report_sha256']
   report=np.load(data/'report.npz')['report'];evaluate('report',report)
   full=[nll(w,28672) for w in report];short=[nll(w[-4097:]) for w in report]
   assert all(math.isfinite(x) for x in full+short)
   save(out/'context-diagnostic.json',dict(utc=utc(),full_context_tail_nll=full,short_context_tail_nll=short,benefit_nats=[s-f for s,f in zip(short,full)],scope='Same final4096 targets, full32K versus last4K context. Positions reset for short input. Descriptive context-sensitivity diagnostic, not a long-range reasoning benchmark.'))
   assert sha(data/'tasks.json')==cfg['task_metadata_sha256'] and sha(data/'tasks.npz')==cfg['task_tokens_sha256']
   meta=json.loads((data/'tasks.json').read_text());task_arrays=np.load(data/'tasks.npz');flat=task_arrays['input_ids'];offsets=task_arrays['offsets']
   ...
   save(out/'task-results.json',dict(predictions=predictions,checkpoint_sha256=sha(a.resume),step=step,task_metadata_sha256=cfg['task_metadata_sha256'],task_tokens_sha256=cfg['task_tokens_sha256']))

  else:
   if a.resume:
    step,extra=restore_checkpoint(a.resume,params,optimizer,scheduler,identity);rows=extra['rows'];evaluations=extra['evaluations']
   else:
    evaluate('calibration',cal);checkpoint()
  ...
  result=dict(status='complete',phase=a.phase,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),wall_seconds=time.perf_counter()-tick,step=step,
   ...
   initial_sha256=initial_sha,training_seconds=sum(x['seconds'] for x in rows),trace=rows,evaluations=evaluations,gates=gates,peak_gpu_bytes=torch.cuda.max_memory_allocated(),scope=cfg['scope'],implementation=implementation)
 except Exception:
