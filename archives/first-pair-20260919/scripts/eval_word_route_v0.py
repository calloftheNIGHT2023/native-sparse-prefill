...
def main(a):
 ...
 protocol=json.loads((R/'provenance/word-route-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  state=dict(k=a.k,reference=False,route_mode=None);base.LOCKED_POOL_CONFIG=(32,4,3)
  ...
  def attention(module,query,key,value,attention_mask,scaling=None,dropout=0.,**kw):
   ...
   if state['k']==0:
    ...
    return y.transpose(1,2).to(query.dtype),None
   if state['route_mode'] is not None:
    from word_route_intervention import routed_attention
    return routed_attention(q,k,v,scaling,mode=state['route_mode'],start=state['start'],forced=state['forced'],target=state['target'],telemetry=state['telemetry'])[None].to(query.dtype),None
  ...
  state['route_mode']='rebuild' if job['condition']=='baseline' else 'forced'
  placements=json.loads((R/'data/word-route-v0/placements.json').read_text())
  reference=json.loads((R/job['reference']).read_text())
  assert sha(R/job['reference'])==job['reference_sha256']
  ...
  labels=torch.tensor(protocol['answer_token_ids'],device='cuda');predictions=[];replay_errors=[]
  with torch.no_grad():
   for j,item in enumerate(meta):
    if item['variant']!='long32768':continue
    placement=placements[item['item_id']];state['start']=placement['query_start'];state['target']=[placement['target_block']]
    state['forced']=[placement['target_block'] if job['condition']=='target' else placement['sham_block']];state['telemetry']=[]
    ids=torch.tensor(flat[offsets[j]:offsets[j+1]].astype(np.int64)[None],device='cuda')
    torch.cuda.synchronize();pt=time.perf_counter()
    with amp():h=model.model(ids,use_cache=False).last_hidden_state[:,-1:];logits=model.lm_head(h)[0,0].float()
    values=logits[labels].cpu().tolist();elapsed=time.perf_counter()-pt;assert np.isfinite(values).all();guess=int(np.argmax(values))
    assert len(state['telemetry'])==24
    pred=dict(**item,prediction=guess,correct=guess==item['gold'],choice_logits=values,placement=placement,route_telemetry=state['telemetry'],seconds=elapsed)
    predictions.append(pred)
    with (out/'task-predictions.jsonl').open('a') as f:f.write(json.dumps(pred)+'\n')
    if job['condition']=='baseline':
     old=ref[item['item_id']];err=max(abs(x-y) for x,y in zip(values,old['choice_logits']));replay_errors.append(err)
     assert err<=protocol['task_replay_max_abs_error'] and guess==old['prediction'],dict(item=item['item_id'],error=err)
    if len(predictions)%16==0:event('prediction_progress',completed=len(predictions),maximum=64,condition=job['condition'])
  assert len(predictions)==64
  result=dict(status='complete',job=job,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,step=step,optimizer_updates=0,task_predictions=len(predictions),calibration_values=vals,calibration_max_abs_error=error,checkpoint_sha256=sha(checkpoint),eval_source_sha256=sha(Path(__file__)),predictions=predictions,task_replay_max_abs_error=max(replay_errors) if replay_errors else None)
 except Exception:
