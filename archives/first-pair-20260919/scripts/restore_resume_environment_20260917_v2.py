...
R=Path('/workspace/native-sparse-prefill/amp-confirmation-20260915-v0');out=Path('/opt/nsp-resume-environment-v1');env=Path('/opt/nsp-resume-env-v2')
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def main():
 out.mkdir(exist_ok=False);started=datetime.now(timezone.utc).isoformat();tick=time.monotonic();state=out/'state.json'
 ...
 try:
  ...
  with (out/'install.log').open('w') as f:subprocess.run([py,'-m','pip','install','--cache-dir','/tmp/nsp-resume-pip-cache','--no-deps','--progress-bar','off','--extra-index-url','https://download.pytorch.org/whl/cu128','-r',str(pins)],stdout=f,stderr=subprocess.STDOUT,check=True,timeout=1500)
