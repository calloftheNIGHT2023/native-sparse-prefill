...
from task_route_additive import routed_attention
...
def main(a):
 ...
 route_mode='calibration';suffix_start=None;forced=None;target_blocks=None;telemetry=[];sham_candidates=None
 def attention(module,query,key,value,attention_mask,scaling=None,dropout=0.,**kw):
  ...
  out=routed_attention(q,kk,v,scaling,mode='rebuild' if route_mode=='baseline' else ('add_target' if route_mode=='target' else 'add_sham'),start=suffix_start,forced=forced,target=target_blocks,telemetry=telemetry,sham_candidates=sham_candidates)
 ...
 assert k==16 and read(R/'results/task-route-additive-preflight-v0/result.json')['status']=='passed'
 route_protocol=read(R/'provenance/task-quality-route-additive-protocol.json')
 assert sha(R/'scripts/task_route_additive.py')==route_protocol['route_source_sha256']
 ...
 for condition in route_protocol['order'][str(seed)]:
  ...
  for j,(i,row) in enumerate(rows):
   placement=placements[row['item_id']];sham_candidates=placement['sham_candidates'];suffix_start=placement['suffix_start'];target_blocks=placement['target_blocks'];forced=placement['sham_blocks'] if condition=='sham' else target_blocks
   if j==0:
   ...
   entry=dict(block_budget_at_suffix=16 if condition=='baseline' else 16+len(target_blocks),condition=condition,item_id=row['item_id'],gold=row['gold'],prediction=guess,correct=guess==row['gold'],choice_logits=values,prefill_seconds=seconds,layer_telemetry=list(telemetry),suffix_start=suffix_start,target_blocks=target_blocks,forced_blocks=target_blocks if condition=='target' else [],sham_candidates=sham_candidates if condition!='baseline' else [])
