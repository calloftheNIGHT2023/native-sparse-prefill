...
def main():
    name='lambada-natural';a=R/f'exports/{name}-evidence-v0.tar.gz';proof=load(a.with_suffix('.json'));assert sha(a)==proof['sha256'] and a.stat().st_size==proof['bytes'];dest=R/f'results/cloud-{name}-evidence-v0';dest.mkdir(exist_ok=True)
    ...
    for j in p['jobs'][:len(control['jobs'])]:
        ...
        assert v['eval_source_sha256']==sha(dest/'scripts/eval_lambada_natural_v0.py')==sha(d/'source.py') and v['checkpoint_sha256']==j['sha256']
