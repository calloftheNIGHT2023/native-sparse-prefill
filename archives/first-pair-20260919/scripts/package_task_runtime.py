...
files=set()
for sub in ['results/task-quality-stage-v0','results/task-runtime-stage-v0','data/task-quality-v0']:
 files.update(p for p in (R/sub).rglob('*') if p.is_file())
for pattern in ['scripts/*task*.py','scripts/amp_recovery_config_identity.py','scripts/run_amp_recovery.py','scripts/amp_recovery_state.py','scripts/chunked_lm_loss.py','scripts/run_flashmoba_realtext_precision.py','docs/task-quality-protocol-2026-09-15.md','docs/task-runtime-protocol-2026-09-15.md','provenance/task-quality*']:
 files.update(p for p in R.glob(pattern) if p.is_file())
a=R/'exports/task-runtime-evidence-v0.tar.gz';assert not a.exists();rows=[]
