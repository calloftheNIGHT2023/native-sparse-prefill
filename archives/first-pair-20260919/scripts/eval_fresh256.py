...
def source_identity():
 return {n:sha(R/'scripts'/n) for n in ['run_expanded76.py','amp_recovery_state.py','chunked_lm_loss.py','run_flashmoba_realtext_precision.py']}
def main(a):
 import run_flashmoba_realtext_precision as base
 data=R/'data/32k-expanded-training-v0';cfg=json.loads((data/'config.json').read_text())
 assert a.phase=='report'
 fresh=R/'data/fresh256-confirmation-v0';fm=json.loads((fresh/'manifest.json').read_text())
 protocol=json.loads((R/'provenance/fresh256-confirmation-protocol.json').read_text())
 ...
 try:
  ...
  if a.phase=='preflight':
   ...
   assert sha(fresh/'tasks.json')==fm['task_metadata_sha256'] and sha(fresh/'tasks.npz')==fm['task_tokens_sha256']
   meta=json.loads((fresh/'tasks.json').read_text());task_arrays=np.load(fresh/'tasks.npz');flat=task_arrays['input_ids'];offsets=task_arrays['offsets']
   ...
   save(out/'task-results.json',dict(predictions=predictions,checkpoint_sha256=sha(a.resume),step=step,task_metadata_sha256=fm['task_metadata_sha256'],task_tokens_sha256=fm['task_tokens_sha256']))
  ...
  result=dict(status='complete',phase=a.phase,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),wall_seconds=time.perf_counter()-tick,step=step,
   ...
   initial_sha256=initial_sha,training_seconds=sum(x['seconds'] for x in rows),trace=rows,evaluations=evaluations,gates=gates,peak_gpu_bytes=torch.cuda.max_memory_allocated(),scope=fm['scope'])
 except Exception:
