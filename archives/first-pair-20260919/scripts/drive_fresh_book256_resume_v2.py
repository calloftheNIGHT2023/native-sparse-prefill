...
def main():
 c=json.loads((R/'logs/cloud-connection-current.json').read_text());assert c['pod_id']=='xc2y1iadcrgy31';opts=['-i',c['ssh_key_path'],'-o','BatchMode=yes','-o','ConnectTimeout=15'];host=c['ssh_user']+'@'+c['ssh_host'];ssh=['ssh',*opts,'-p',str(c['ssh_port']),host];scp=['scp',*opts,'-P',str(c['ssh_port'])];state=R/'logs/fresh-book256-resume-driver-v2.json';start=time.monotonic()
 ...
 try:
  while time.monotonic()-start<2100:
   active();v=remote('from pathlib import Path\nimport json\nr=Path('+repr(c['root'])+')\nprint((Path("/opt/nsp-resume-environment-v1/state.json")).read_text())');save(state,dict(status='waiting_environment',utc=utc(),environment=v))
  ...
  envproof=R/'logs/resume-environment-v2-proof';envproof.mkdir(exist_ok=True)
  for n in ['state.json','result.json','install.log','extension.log','source.py']:subprocess.run(scp+[host+':/opt/nsp-resume-environment-v1/'+n,str(envproof/n)],check=True,timeout=60)
  cleanup=remote("from pathlib import Path\nprint(Path('/tmp/nsp-quota-cleanup-20260917.json').read_text())");assert cleanup['status']=='complete';save(R/'logs/resume-environment-quota-cleanup-v0.json',cleanup)
