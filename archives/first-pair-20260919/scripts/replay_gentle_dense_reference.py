...
def main(a):
 ...
 try:
  ...
  if a.phase=='preflight':
   ...
   step,extra=restore_checkpoint(a.resume,params,optimizer,scheduler,identity);assert step==128
   ...
   save(out/'replay-verification.json',dict(step=step,max_abs_error=replay_error,checkpoint_sha256=sha(a.resume),calibration_values=calibration_replay,utc=utc(),optimizer_updates=0,task_predictions=0))
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--seed',type=int,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 a.phase='report';a.k=0;a.lr=.001;a.preflight=R/'results/expanded76-stage-v0/preflight-k0'
 a.resume=R/f'results/expanded76-stage-v0/k0-seed{a.seed}/checkpoint-128.pt';a.selection=None
 main(a)
