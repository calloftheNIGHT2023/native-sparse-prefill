...
def main(a):
 ...
 def attention(module,query,key,value,attention_mask,scaling=None,dropout=0.,**kw):
  ...
  y,mass=routed_attention(q,kk,v,scaling,route_mode,suffix_start,forced,target_blocks,heads)
  if route_mode=='baseline':head_map[layer]=dict(mass=mass,selected=select_heads(mass,4))
 ...
 route_protocol=read(R/'provenance/task-quality-route-selective-protocol.json')
 ...
 result=dict(status='complete',seed=seed,k=k,lr=lr,optimizer_updates=0,checkpoint_sha256=index['sha256'],
             ...
             control_max_logit_error=max(control_errors),all_target_max_logit_error=max(all_errors),
             route_protocol_sha256=sha(R/'provenance/task-quality-route-selective-protocol.json'),
