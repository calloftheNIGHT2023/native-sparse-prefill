...
def main():
    pp=R/'provenance/fresh-book256-resume-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/fresh-book256-resume-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/fresh-book256-resume-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_fresh_book256_resume_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
            ...
            v=json.loads((out/j['name']/'result.json').read_text())
            if j['phase']=='base_gate' and not v['base_gate']['passed']:
                break
    ...
    if status=='complete':assert count==3087
    gradient_count=0
    if status=='complete':assert gradient_count==p['expected_gradient_passes']
    new_count=sum(json.loads((out/j['name']/'result.json').read_text()).get('new_task_predictions',0) for j in jobs if (out/j['name']/'result.json').exists()); replay_count=sum(json.loads((out/j['name']/'result.json').read_text()).get('diagnostic_replay_predictions',0) for j in jobs if (out/j['name']/'result.json').exists())
    save(out/'result.json',dict(new_task_predictions=new_count,diagnostic_replay_predictions=replay_count,gradient_passes=gradient_count,status=status,started_utc=start,finished_utc=utc(),seconds=time.monotonic()-tick,jobs=jobs,optimizer_updates=0,task_predictions=count,protocol_sha256=sha(pp)))
    archive=R/'exports/fresh-book256-resume-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('fresh-book256-resume-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
