...
def main(a):
 ...
 protocol=json.loads((R/'provenance/word-operator-swap-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  state['k']=job['evaluation_k'];assert state['k'] in [0,32] and state['k']!=a.k
