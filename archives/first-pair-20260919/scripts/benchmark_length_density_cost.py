...
def main(a):
 ...
 cost_protocol=read(R/'provenance/length-density-cost-protocol.json')
 train=np.load(calfile)['train'];rows=[]
 for name,h in cost_protocol['sources'].items():assert sha(R/name)==h
 assert int(model.config.max_position_embeddings)>=32768
 ...
 for round_index,order in enumerate(cost_protocol['orders']):
  wi=cost_protocol['window_pairs'][round_index]
  full=np.concatenate([train[wi[0]],train[wi[1]][1:]])
  assert len(full)==32769
  lengths=cost_protocol['length_orders'][round_index]
  for length in lengths:
   ...
   for mode in order:
    ...
    row=dict(round=round_index,length=length,k=mode,window_pair=wi,input_sha256=hashlib.sha256(w.tobytes()).hexdigest(),times=times,losses=losses,gradient_norms=norms,median_seconds=float(np.median(times)),peak_bytes=torch.cuda.max_memory_allocated(),utc=datetime.now(timezone.utc).isoformat())
    rows.append(row)
 ...
 for length in [16384,32768]:
  for mode in [16,32]:
   ratios=[next(x['median_seconds'] for x in rows if x['length']==length and x['k']==mode and x['round']==r)/next(x['median_seconds'] for x in rows if x['length']==length and x['k']==0 and x['round']==r) for r in [0,1]]
   candidates.append(dict(length=length,k=mode,ratios=ratios))
 result=dict(status='complete',started_utc=started,finished_utc=datetime.now(timezone.utc).isoformat(),seconds=time.monotonic()-tick,rows=rows,candidates=candidates,optimizer_updates=0,weights_unchanged=True,initial_params_sha256=initial,checkpoint_sha256=index['sha256'],calibration_replay_max_abs_error=replay_error,protocol_sha256=sha(R/'provenance/length-density-cost-protocol.json'),scope=cost_protocol['scope'])
