...
def main(a):
 ...
 protocol=read(R/'provenance/task-quality-factorial-protocol.json');condition=protocol['conditions'][a.condition]
 assert a.run_dir.resolve()==Path(condition['run_dir']).resolve()
 assert sha(a.run_dir/'result.json')==condition['training_result_sha256'] and index['sha256']==condition['checkpoint_sha256']
 ...
 assert dict(k=k,seed=seed,lr=lr)==condition['identity']
 ...
 assert a.split=='formal' and seed==2026091560
 if condition['weights_mode']=='base_without_adapters':
  # Remove wrappers entirely after checkpoint calibration replay. Frozen original weights are unchanged.
  for layer in model.model.layers:
   for name in ['q_proj','k_proj','v_proj','o_proj']:
    wrapper=getattr(layer.self_attn,name);assert isinstance(wrapper,LoRALinear);setattr(layer.self_attn,name,wrapper.base)
  assert not any(isinstance(m,LoRALinear) for m in model.modules())
 k=condition['task_attention_k']
 controls={}
 if condition['control_result']:
  controls={(x['item_id'],x['variant']):x for x in read(R/condition['control_result'])['predictions']}
 control_errors=[]
 if a.split=='pilot':
 ...
 for i,row in enumerate(metadata):
  ...
  if controls:
   expected=controls[row['item_id'],row['variant']];error=max(abs(x-y) for x,y in zip(values,expected['choice_logits']))
   assert guess==expected['prediction'] and error<=1e-6,(a.condition,row['item_id'],error);control_errors.append(error)
 ...
 assert len(predictions)==384
 result=dict(condition=a.condition,weights_mode=condition['weights_mode'],source_training_identity=condition['identity'],control_max_logit_error=max(control_errors) if control_errors else None,diagnostic_protocol_sha256=sha(R/'provenance/task-quality-factorial-protocol.json'),status='complete',split=a.split,k=k,seed=seed,lr=lr,started_utc=started,finished_utc=datetime.now(timezone.utc).isoformat(),seconds=time.monotonic()-tick,
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--condition',required=True);p.add_argument('--run-dir',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--split',choices=['pilot','formal'],required=True);p.add_argument('--gate',type=Path);main(p.parse_args())
