...
def main():
    pp=R/'provenance/checkpointed-training-cost-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/checkpointed-training-cost-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/checkpointed-training-cost-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/benchmark_checkpointed_training_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    gradient_count=sum(len(f.read_text().splitlines()) for f in out.glob('*/gradient-passes.jsonl'))
    ...
    archive=R/'exports/checkpointed-training-cost-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('checkpointed-training-cost-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
