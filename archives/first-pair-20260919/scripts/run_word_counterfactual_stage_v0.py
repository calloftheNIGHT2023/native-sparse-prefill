...
def main():
    pp=R/'provenance/word-counterfactual-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/word-counterfactual-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/word-counterfactual-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_word_counterfactual_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
            ...
            with (out/f"{j['name']}.log").open('w') as f:
                ...
                try:rc=q.wait(timeout=remaining)
    ...
    if status=='complete':assert count in [p['expected_task_predictions_if_short_passes'],p['expected_task_predictions_if_short_fails']]
    save(out/'result.json',dict(status=status,started_utc=start,finished_utc=utc(),seconds=time.monotonic()-tick,jobs=jobs,optimizer_updates=0,task_predictions=count,protocol_sha256=sha(pp)))
    archive=R/'exports/word-counterfactual-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('word-counterfactual-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
