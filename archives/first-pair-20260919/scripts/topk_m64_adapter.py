...
def enable(base):
 p=R/'experiments/topk-m64-v0/build-result.json';v=json.loads(p.read_text());assert v['status']=='built'
 binary=Path(v['module_path']);assert hashlib.sha256(binary.read_bytes()).hexdigest()==v['module_sha256']
 spec=importlib.util.spec_from_file_location('nsp_topk_m64_v0',binary);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
