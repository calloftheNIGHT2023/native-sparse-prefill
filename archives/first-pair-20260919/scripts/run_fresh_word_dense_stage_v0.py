...
def main():
    pp=R/'provenance/fresh-word-dense-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/fresh-word-dense-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/fresh-word-dense-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_fresh_word_dense_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
            ...
            if j['phase']=='base_gate' and not (v['short_gate']['passed'] and v['long_gate'] and v['long_gate']['passed']):
                break
    ...
    if status=='complete':assert count in [5,133,p['expected_task_predictions']]
    gradient_count=0
    ...
    archive=R/'exports/fresh-word-dense-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('fresh-word-dense-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
