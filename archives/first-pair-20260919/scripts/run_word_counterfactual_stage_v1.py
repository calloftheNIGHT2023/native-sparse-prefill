...
def main():
    pp=R/'provenance/word-counterfactual-protocol-v1.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/word-counterfactual-stage-v1';out.mkdir(exist_ok=False);state=R/'logs/word-counterfactual-queue-v1.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_word_counterfactual_v1.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/word-counterfactual-evidence-v1.tar.gz';assert not archive.exists();entries=[]
