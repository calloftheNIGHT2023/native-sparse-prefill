...
def main():
    pp=R/'provenance/book256-factorial-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/book256-factorial-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/book256-factorial-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_book256_factorial_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/book256-factorial-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('book256-factorial-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
