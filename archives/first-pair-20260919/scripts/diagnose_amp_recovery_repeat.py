...
import argparse,hashlib,json,math,random,time,traceback
...
def main(a):
 ...
 started=utc();tick=time.perf_counter();rows=[];evaluations=[];step=0;local_updates=0
 ...
 try:
  ...
  if a.phase=='preflight':
   import functools
   diagnostic=[]
   original_backward=base.flash_moba_attn_varlen_func
   for deterministic in [False,True]:
    torch.use_deterministic_algorithms(deterministic)
    base.flash_moba_attn_varlen_func=functools.partial(original_backward,deterministic=deterministic)
    for k in [0,4,16]:
     state['k']=k;ls=[];gs=[]
     for j in range(3):
      ls.append(float(backward(train[0])));gs.append(gradients())
     row=dict(deterministic=deterministic,k=k,losses=ls,gradient_repeat_max_abs=[float((g-gs[0]).abs().max()) for g in gs[1:]],gradient_relative_l2=[float((g-gs[0]).norm()/gs[0].norm()) for g in gs[1:]])
     diagnostic.append(row);event('repeat_diagnostic',**row);save(out/'partial-diagnostic.json',diagnostic)
     del gs
   save(out/'result.json',dict(status='complete',scope='Repeated full16K backward at identical weights, zero optimizer updates; no quality measurement',started_utc=started,finished_utc=utc(),rows=diagnostic,environment=environment,identity=identity))
   return
  elif a.phase=='report':
  ...
  result=dict(status='failed',phase=a.phase,step=step,optimizer_updates_this_process=local_updates,started_utc=started,finished_utc=utc(),trace=rows,evaluations=evaluations,error=traceback.format_exc());event('failure',error=result['error'])
