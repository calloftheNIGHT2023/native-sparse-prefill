...
def main():
 pp=R/'provenance/density-tradeoff-cost-protocol.json';p=json.loads(pp.read_text());statefile=R/'logs/density-tradeoff-cost-queue-v0.json';start=time.monotonic()
 ...
 out=R/'results/density-tradeoff-cost-stage-v0';out.mkdir(exist_ok=False);tick=time.monotonic();started=utc();jobs=[];failed=set()
 ...
 try:
  for ri,order in enumerate(p['orders']):
   for mode in order:
    ...
    dest=out/name;cmd=[sys.executable,'-u',str(R/'scripts/benchmark_density_tradeoff_cost.py'),'--phase','train','--output',str(dest),'--k','0','--seed','2026091660','--lr','0.001','--mode',str(mode),'--round',str(ri),'--preflight',str(R/'results/32k-adaptation-stage-v0/preflight-k0'),'--resume',str(R/p['parent_checkpoint'])]
 ...
 for mode in [32,48,64]:
  rows=[x for x in measurements if x['mode']==mode];ratios=[]
  for x in rows:
 ...
 a=R/'exports/density-tradeoff-cost-evidence-v0.tar.gz';assert not a.exists();entries=[]
 ...
 with tarfile.open(a,'w:gz',compresslevel=1) as t:
  ...
  raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('density-tradeoff-cost-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
...
if __name__=='__main__':
 try:main()
  ...
  save(R/'logs/density-tradeoff-cost-queue-v0.json',dict(status='failed',utc=utc(),error=traceback.format_exc()))
  raise
