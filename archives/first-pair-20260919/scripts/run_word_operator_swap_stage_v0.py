...
def main():
    pp=R/'provenance/word-operator-swap-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/word-operator-swap-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/word-operator-swap-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_word_operator_swap_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/word-operator-swap-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('word-operator-swap-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
