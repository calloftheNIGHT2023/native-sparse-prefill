...
def main(a):
 ...
 protocol=json.loads((R/'provenance/vo-training-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  frozen={n:p for n,p in params.items() if any('.'+q+'.' in n for q in ['q_proj','k_proj'])}
  trainable={n:p for n,p in params.items() if n not in frozen};assert len(frozen)==len(trainable)==96 and sum(p.numel() for p in trainable.values())==540672
  for p in frozen.values():p.requires_grad_(False)
  ...
  new_identity=dict(parent=identity,training_k=state['k'],trainable_projection_names=['v_proj','o_proj'],trainable_parameters=540672,protocol_sha256=sha(R/'provenance/vo-training-protocol-v0.json'),gradient_checkpointing='nonreentrant')
  rows=[];evaluations=[]
  def checkpoint():
  ...
  report=np.load(R/'data/32k-expanded-training-v0/report.npz')['report'];report_values=[nll(w) for w in report];assert len(report_values)==9
  arrays=np.load(R/'data/word-counterfactual-v0/tasks.npz');meta=json.loads((R/'data/word-counterfactual-v0/tasks.json').read_text());flat=arrays['input_ids'];offsets=arrays['offsets'];labels=torch.tensor(protocol['answer_token_ids'],device='cuda');predictions=[]
  with torch.no_grad():
   for j,item in enumerate(meta):
    ...
    values=logits[labels].cpu().tolist();assert np.isfinite(values).all();guess=int(np.argmax(values));pred=dict(**item,prediction=guess,correct=guess==item['gold'],choice_logits=values);predictions.append(pred)
    with (out/'task-predictions.jsonl').open('a') as f:f.write(json.dumps(pred)+'\n')
  assert len(predictions)==192 and all(math.isfinite(x) for x in report_values) and digest(frozen)==frozen_digest
  result=dict(status='complete',job=job,identity=new_identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,training_completed_seconds=training_completed_seconds,step=step,optimizer_updates=local_updates,task_predictions=192,gradient_gate_passes=2,gradient_gate=gate,rows=rows,evaluations=evaluations,loop_seconds=loop_seconds,complete_step_seconds=sum(x['seconds'] for x in rows),peak_allocated_bytes=peak,calibration_values=vals,calibration_max_abs_error=error,checkpoint_sha256=sha(checkpoint_path),checkpoint_hashes={str(i):sha(out/f'checkpoint-{i}.pt') for i in [0,64,128]},eval_source_sha256=sha(Path(__file__)),frozen_qk_unchanged=True,frozen_qk_digest=frozen_digest,initial_vo_digest=initial_vo_digest,final_vo_digest=digest(trainable),report_nll=report_values,predictions=predictions)
