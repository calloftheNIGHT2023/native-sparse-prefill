...
def main(a):
 ...
 attention_k=k;dense_layers=None;seen_layers=set()
 ...
 def attention(module,query,key,value,attention_mask,scaling=None,dropout=0.,**kw):
  ...
  layer=int(module.layer_idx);assert 0<=layer<24;seen_layers.add(layer)
  mode=attention_k if dense_layers is None else (0 if layer in dense_layers else 16)
  if mode==0:
  ...
  return base.sparse(q,kk,v,128,mode,'fp32',scaling)[None].to(query.dtype),None
 ...
 assert a.split=='formal' and k==16
 layer_protocol=read(R/'provenance/task-quality-layer-protocol.json')
 ...
 rows=[(i,row) for i,row in enumerate(metadata) if row['task']=='race_mc' and row['variant']=='long16384'];assert len(rows)==96
 seedindex=seed-2026091560
 controls={name:{x['item_id']:x for x in read(R/path.format(seed=seedindex))['predictions'] if x['variant']=='long16384'} for name,path in layer_protocol['controls'].items()}
 all_predictions=[];control_checks=[];conditions=layer_protocol['conditions'];order=layer_protocol['order'][str(seed)]
 for name in order:
  dense_layers=set(conditions[name]);seen_layers.clear()
  for _ in range(2):score(flat[offsets[rows[0][0]]:offsets[rows[0][0]+1]])
  predictions=[]
  for i,row in rows:
   guess,values,mass,seconds=score(flat[offsets[i]:offsets[i+1]])
   entry=dict(condition=name,item_id=row['item_id'],gold=row['gold'],prediction=guess,correct=guess==row['gold'],choice_logits=values,prefill_seconds=seconds)
   if name in controls:
    expected=controls[name][row['item_id']];error=max(abs(x-y) for x,y in zip(values,expected['choice_logits']))
    assert guess==expected['prediction'] and error<=1e-6,(name,row['item_id'],error)
    control_checks.append(error)
   predictions.append(entry)
   with (a.output/'predictions.jsonl').open('a') as f:f.write(json.dumps(entry)+'\n')
  assert seen_layers==set(range(24))
  all_predictions.extend(predictions);print(json.dumps(dict(event='condition_complete',condition=name,correct=sum(x['correct'] for x in predictions),total=96)),flush=True)
 result=dict(status='complete',seed=seed,k=k,lr=lr,checkpoint_sha256=index['sha256'],training_result_sha256=sha(a.run_dir/'result.json'),task_protocol_sha256=sha(taskdir/'protocol.json'),layer_protocol_sha256=sha(R/'provenance/task-quality-layer-protocol.json'),calibration_replay_max_abs_error=replay_error,control_max_logit_abs_error=max(control_checks),predictions=all_predictions,optimizer_updates=0,started_utc=started,finished_utc=datetime.now(timezone.utc).isoformat(),seconds=time.monotonic()-tick)
 (a.output/'result.json').write_text(json.dumps(result,indent=2)+'\n');(a.output/'source.py').write_bytes(Path(__file__).read_bytes())
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--run-dir',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--split',default='formal');p.add_argument('--gate',type=Path,required=True);main(p.parse_args())
