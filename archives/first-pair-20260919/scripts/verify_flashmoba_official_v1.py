"""Gate v1: router checked conditional on actual pooled keys; report ideal FP32-pool differences separately.
...
def block_masks(q, k, lengths, block, topk, provided_means=None):
    """Independent mean routing; current block mandatory, only completed remote blocks."""
    masks=[]; scores=[]; start=0; mean_start=0
    for n in lengths:
        ...
        if provided_means is not None:
            means=provided_means[mean_start:mean_start+math.ceil(n/block)]; mean_start+=math.ceil(n/block)
        ...
        for dtype in [torch.float16,torch.bfloat16]:
            for lengths,h,hk,d,block,topk in cases:
                ...
                with torch.no_grad():
                    ...
                    pooled,_,_=flash_topk_mean_pool(k,cu,max(lengths),block)
                    ideal_masks,_=block_masks(q,k,lengths,block,topk)
                    independent,scores=block_masks(q,k,lengths,block,topk,provided_means=pooled)
                    ideal_disagreements=sum(int((m!=r).any(-1).sum()) for m,r in zip(masks,ideal_masks))
                ...
                row=dict(dtype=str(dtype),lengths=lengths,heads=h,kv_heads=hk,dim=d,block=block,topk_including_current=topk,
                    ...
                    changed_rows_vs_ideal_fp32_pool=ideal_disagreements,routing_gate_conditions_on_actual_official_means=True,
                    output_max_abs=float((actual.float()-expected).detach().abs().max()),
