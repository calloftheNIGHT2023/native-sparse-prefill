...
def main(a):
 ...
 protocol=json.loads((R/'provenance/checkpointed-training-cost-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  def digest():return hashlib.sha256(b''.join(p.detach().cpu().numpy().tobytes() for p in params.values())).hexdigest()
  initial_digest=digest();rows=[];gates=[];passes=0
  def compute(w,k,checkpointing,phase,round_index=-1,repeat=-1,window=0):
   nonlocal passes
   state['k']=k
   ...
   assert model.model.gradient_checkpointing==checkpointing
   ids,target=inputs(w);model.zero_grad(set_to_none=True);torch.cuda.synchronize();torch.cuda.reset_peak_memory_stats()
   ev=[torch.cuda.Event(enable_timing=True) for _ in range(4)];pt=time.perf_counter();ev[0].record()
   with amp():h=model.model(ids,use_cache=False).last_hidden_state
   ev[1].record();loss=chunked_head_backward(h,model.lm_head,target,cfg['chunk_size'],amp);ev[2].record()
   norm=torch.nn.utils.clip_grad_norm_(list(params.values()),cfg['gradient_clip'],error_if_nonfinite=True);ev[3].record();torch.cuda.synchronize()
   wall=time.perf_counter()-pt;peak=torch.cuda.max_memory_allocated();values=[ev[i].elapsed_time(ev[i+1]) for i in range(3)];assert bool(torch.isfinite(loss)) and bool(torch.isfinite(norm))
   g=torch.cat([p.grad.detach().float().flatten() for p in params.values()]).cpu();assert torch.isfinite(g).all();passes+=1
   row=dict(phase=phase,round=round_index,repeat=repeat,window=window,k=k,checkpointing=checkpointing,tokens=len(w)-1,input_sha256=hashlib.sha256(w.tobytes()).hexdigest(),loss=float(loss),gradient_norm=float(norm),gradient_sha256=hashlib.sha256(g.numpy().tobytes()).hexdigest(),seconds=wall,forward_cuda_ms=values[0],head_and_backbone_backward_cuda_ms=values[1],clip_cuda_ms=values[2],peak_allocated_bytes=peak,utc=utc())
   rows.append(row)
   with (out/'gradient-passes.jsonl').open('a') as f:f.write(json.dumps(row)+'\n')
   event('gradient_pass',completed=passes,**row)
   del h,ids,target,loss,norm
   model.zero_grad(set_to_none=True)
   return row,g
  for k in [0,32]:
   x,g=compute(train[0,:2049],k,False,'math_gate');y,h=compute(train[0,:2049],k,True,'math_gate');rel=float((h-g).norm()/g.norm().clamp_min(1e-12));gap=abs(x['loss']-y['loss'])
   gate=dict(k=k,loss_gap=gap,gradient_relative_l2=rel,passed=gap<.001 and rel<.02);gates.append(gate);save(out/'gates.json',gates);assert gate['passed'],gate;del g,h
  for k in [0,32]:compute(train[0],k,True,'warmup')
  for rep,order in enumerate(protocol['orders']):
   wi=protocol['windows'][rep]
   for k in order:
    x,g=compute(train[wi],k,True,'timed',rep,0,wi);y,h=compute(train[wi],k,True,'timed',rep,1,wi)
    maximum=float((g-h).abs().max());passed=torch.allclose(g,h,atol=1e-6,rtol=1e-4) and abs(x['loss']-y['loss'])<.001
    gate=dict(name='repeat_gradient',round=rep,k=k,max_abs_error=maximum,loss_gap=abs(x['loss']-y['loss']),passed=passed);gates.append(gate);save(out/'gates.json',gates);assert passed,gate;del g,h
  assert passes==18 and initial_digest==digest()
  result=dict(status='complete',job=job,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,step=step,optimizer_updates=0,task_predictions=0,gradient_passes=passes,rows=rows,gates=gates,weights_unchanged=True,initial_parameter_digest=initial_digest,final_parameter_digest=digest(),calibration_values=vals,calibration_max_abs_error=error,checkpoint_sha256=sha(checkpoint),eval_source_sha256=sha(Path(__file__)))
 except Exception:
