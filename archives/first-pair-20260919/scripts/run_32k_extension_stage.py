...
def main():
 out=R/'results/32k-extension-stage-v0';out.mkdir(exist_ok=False)
 pp=R/'provenance/32k-extension-protocol.json';p=json.loads(pp.read_text());started=utc();tick=time.monotonic();jobs=[]
 def run(name,k,seed,phase,resume,selection=None):
  ...
  cmd=[sys.executable,'-u',str(R/'scripts/run_32k_extension.py'),'--phase',phase,'--k',str(k),'--seed',str(seed),'--lr','0.001','--output',str(out/name),'--resume',str(resume),'--preflight',str(parent/f'preflight-k{k}')]
 ...
 try:
  ...
  def target(name,k,seed,step,path,origin):
   j=p['parents'][f'k{k}-seed{seed}'];identity=j['resume_identity']
   if origin=='continuation':identity=dict(parent_identity=identity,continuation_protocol_sha256=sha(pp),parent_checkpoint_sha256=j['checkpoint_sha256'])
   reports.append(dict(name=name,k=k,seed=seed,step=step,checkpoint=str(path.relative_to(R)),checkpoint_sha256=sha(path),origin=origin,identity=identity))
  for k,seed in p['run_order']:
   key=f'k{k}-seed{seed}'
   target(f'report-{key}-step256',k,seed,256,out/key/'checkpoint-256.pt','continuation')
  for seed in p['seeds']:
   ...
   j=p['parents'][f'k0-seed{seed}'];target(f'report-k0-seed{seed}-step128',0,seed,128,R/j['checkpoint'],'parent')
 ...
 archive=R/'exports/32k-extension-evidence-v0.tar.gz';assert not archive.exists()
 ...
 with tarfile.open(archive,'w:gz',compresslevel=1) as tar:
  ...
  raw=json.dumps(dict(files=entries),indent=2).encode();info=tarfile.TarInfo('32k-extension-manifest.json');info.size=len(raw);tar.addfile(info,io.BytesIO(raw))
