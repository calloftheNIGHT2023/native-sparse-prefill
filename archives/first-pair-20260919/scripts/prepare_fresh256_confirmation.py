...
def main():
 out=R/'data/fresh256-confirmation-v0';out.mkdir(exist_ok=True);assert not any(out.iterdir())
 ...
 previous.update(x['article_hash'] for x in json.loads((R/'data/32k-continuation-tasks-v0/tasks.json').read_text(encoding='utf-8')))
 ...
 xs=[(i,dict(x,source_split=split)) for split in ['validation','test'] for i,x in enumerate(tables[split])];random.Random(2026091691).shuffle(xs);selected=[];used=set(previous)
 for num,x in xs:
  h=ah(x['article'])
 ...
 assert len(selected)==256;labels=[enc(' '+c)[0] for c in 'ABCD'];assert all(len(enc(' '+c))==1 for c in 'ABCD')
 flat=[];offsets=[0];meta=[]
 for i,(num,x,h) in enumerate(selected):
  rr=random.Random(2026091692+i);original='ABCD'.index(x['answer']);gold=i%4;wrong=[v for j,v in enumerate(x['options']) if j!=original];rr.shuffle(wrong);options=wrong[:];options.insert(gold,x['options'][original])
  ...
  for variant,tokens in [('short',short),('no_context',prefix+enc('\nTARGET passage: [not provided]\n')+suffix),('long32768',long)]:
   flat.extend(tokens);offsets.append(len(flat));meta.append(dict(item_id=f'fresh256-race-{i:03}',variant=variant,length=len(tokens),gold=gold,source_row=num,source_split=x['source_split'],source_id=x['example_id'],article_hash=h,target_text=x['article'],question=x['question'],options=options,evidence_fraction=fraction,target_token_start=len(prefix)+left if variant=='long32768' else (len(prefix) if variant=='short' else None)))
 ...
 dump(out/'manifest.json',dict(created_utc=datetime.now(timezone.utc).isoformat(),split_article_counts={split:sum(x['source_split']==split for _,x,_ in selected) for split in ['validation','test']},excluded_article_hashes=sorted(previous),source_manifest_sha256=sha(src/'source-manifest.json'),task_metadata_sha256=sha(out/'tasks.json'),task_tokens_sha256=sha(out/'tasks.npz'),label_token_ids=labels,scope='256 new project-unseen RACE middle validation/test articles; pretrained contamination unknown; marked-target extension is not official RACE. Frozen before any fresh256 predictions; earlier training and evaluation outcomes informed this validation design. Prior model pretraining overlap remains unknown.'))
 (out/'preparation-source.py').write_bytes(Path(__file__).read_bytes())
 print(json.dumps(dict(status='prepared',items=256,variants=768)))
