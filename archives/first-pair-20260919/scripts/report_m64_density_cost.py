...
def main():
 a=R/'exports/m64-density-cost-evidence-v0.tar.gz';proof=load(a.with_suffix('.json'));assert sha(a)==proof['sha256'] and a.stat().st_size==proof['bytes']
 dest=R/'results/cloud-m64-density-cost-evidence-v0';dest.mkdir(exist_ok=True)
 with tarfile.open(a) as t:
  entries=json.loads(t.extractfile('m64-density-cost-manifest.json').read())['files'];names=[x.name for x in t.getmembers()];assert len(names)==len(set(names)) and set(names)=={x['path'] for x in entries}|{'m64-density-cost-manifest.json'} and len(entries)==proof['files']
  for e in entries:
 ...
 p=load(dest/'provenance/m64-density-cost-protocol.json');psha=sha(dest/'provenance/m64-density-cost-protocol.json');assert psha==sha(R/'provenance/m64-density-cost-protocol.json')
 ...
 stage=dest/'results/m64-density-cost-stage-v0';control=load(stage/'result.json');rows=[];count=0;order=np.random.default_rng(2026091662).permutation(32).tolist();paired={}
 for j in control['jobs']:
  ...
  assert sha(d/'diagnostic-final.pt')==m['diagnostic_checkpoint_sha256'] and sha(d/'source.py')==p['source_sha256']['scripts/benchmark_m64_density_cost.py']
 ...
 save(R/'results/m64-density-cost-audit-v0/result.json',result);save(dest/'LOCAL-VERIFICATION.json',dict(status='verified',archive=proof,checkpoints=count))
 ...
 (R/'docs/m64-density-cost-results-2026-09-16.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
