...
def main():
    pp=R/'provenance/midpoint64-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/midpoint64-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/midpoint64-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/eval_midpoint64.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    archive=R/'exports/midpoint64-evidence-v0.tar.gz';assert not archive.exists();entries=[]
