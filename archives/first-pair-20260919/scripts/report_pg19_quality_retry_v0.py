...
def main():
    stage='pg19-quality-retry';a=R/f'exports/{stage}-evidence-v0.tar.gz';proof=load(a.with_suffix('.json'))
    ...
    for j in p['jobs']:
        ...
        assert v['eval_source_sha256']==sha(dest/'scripts/eval_pg19_quality_retry_v0.py')==sha(d/'source.py') and v['environment']['gpu']==p['evaluation_gpu'] and v['weights_unchanged']
    ...
    target=R/'results/pg19-quality-retry-audit-v0';target.mkdir(exist_ok=True);(target/'result.json').write_text(json.dumps(result,indent=2)+'\n')
    ...
    (R/'docs/pg19-quality-retry-results-2026-09-17.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
