...
def main(a):
 ...
 protocol=json.loads((R/'provenance/matched-restore-training-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  checkpoint_path=R/job['path'];assert sha(checkpoint_path)==job['sha256']
  ...
  c=torch.load(checkpoint_path,map_location='cpu',weights_only=False);step=c['step'];assert step==job['step'] and c['identity']==identity
  ...
  assert a.k==0 and step==0 and model.config.attention_dropout==0
  frozen={}
  trainable=dict(params);assert len(trainable)==192 and sum(p.numel() for p in trainable.values())==1081344
  def digest(ps):return hashlib.sha256(b''.join(p.detach().cpu().numpy().tobytes() for p in ps.values())).hexdigest()
  frozen_digest=digest(frozen);initial_vo_digest=digest(trainable)
  optimizer=torch.optim.AdamW(list(trainable.values()),lr=.001,betas=tuple(cfg['betas']),eps=cfg['eps'],weight_decay=cfg['weight_decay'],foreach=False,fused=False)
  scheduler=torch.optim.lr_scheduler.LambdaLR(optimizer,lambda s:lr_factor(s,cfg['warmup_steps'],cfg['lr_schedule_steps']))
  state['k']=job['training_k'];assert state['k'] in [0,32]
  ...
  def backward(w):
   ...
   norm=torch.nn.utils.clip_grad_norm_(list(trainable.values()),cfg['gradient_clip'],error_if_nonfinite=True)
   assert bool(torch.isfinite(loss)) and bool(torch.isfinite(norm)) and all(p.grad is None for p in frozen.values())
   return loss,norm
  gs=[];ls=[]
  for repeat in range(2):
   loss,norm=backward(train[order[0]]);gs.append(torch.cat([p.grad.detach().flatten() for p in trainable.values()]).cpu());ls.append(float(loss))
  gate=dict(loss_gap=abs(ls[0]-ls[1]),gradient_max_abs=float((gs[0]-gs[1]).abs().max()),passed=torch.allclose(gs[0],gs[1],atol=1e-6,rtol=1e-4) and abs(ls[0]-ls[1])<.001)
  save(out/'gradient-gate.json',gate);assert gate['passed'];del gs,ls,loss,norm;optimizer.zero_grad(set_to_none=True)
  new_identity=dict(parent=identity,training_k=state['k'],trainable_projection_names=['q_proj','k_proj','v_proj','o_proj'],trainable_parameters=1081344,protocol_sha256=sha(R/'provenance/matched-restore-training-protocol-v0.json'),gradient_checkpointing='nonreentrant')
  rows=[];evaluations=[]
  def checkpoint():
   assert digest(frozen)==frozen_digest
   save_checkpoint(out/f'checkpoint-{step}.pt',params,optimizer,scheduler,step,new_identity,dict(rows=rows,evaluations=evaluations,frozen_qk_digest=frozen_digest))
  @torch.no_grad()
  def evaluate_cal():
   state['k']=0;model.eval();model.gradient_checkpointing_disable();v=[nll(w) for w in cal]
   assert all(math.isfinite(x) for x in v);row=dict(step=step,common_dense_calibration_values=v,utc=utc());evaluations.append(row);save(out/'evaluations.json',evaluations)
   state['k']=job['training_k'];model.train();model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={'use_reentrant':False})
  checkpoint();torch.cuda.reset_peak_memory_stats();loop_start=time.perf_counter()
  for i in range(protocol['steps_per_job']):
   torch.cuda.synchronize();st=time.perf_counter();wi=order[i%76];lr=optimizer.param_groups[0]['lr']
   loss,norm=backward(train[wi]);optimizer.step();scheduler.step();torch.cuda.synchronize();elapsed=time.perf_counter()-st
   ...
   event('optimizer_step',**row);del loss,norm
   if step in [64,128]:
    evaluate_cal();checkpoint()
    if step==64:
     original_digest=digest(params);old_lr=optimizer.param_groups[0]['lr']
     restored,_=restore_checkpoint(out/'checkpoint-64.pt',params,optimizer,scheduler,new_identity)
     assert restored==64 and digest(params)==original_digest and optimizer.param_groups[0]['lr']==old_lr
     event('checkpoint_reload_verified',step=64)
  torch.cuda.synchronize();loop_seconds=time.perf_counter()-loop_start;training_completed_seconds=time.perf_counter()-tick
  assert local_updates==protocol['steps_per_job']==128 and digest(frozen)==frozen_digest
  peak=torch.cuda.max_memory_allocated();state['k']=0;model.eval();model.gradient_checkpointing_disable()
