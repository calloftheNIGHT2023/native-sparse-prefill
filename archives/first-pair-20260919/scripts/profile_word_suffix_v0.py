...
def main(a):
 ...
 protocol=json.loads((R/'provenance/word-suffix-profile-protocol-v0.json').read_text());job=next(x for x in protocol['jobs'] if x['name']==a.job)
 ...
 try:
  ...
  gpu_before=gpu_snapshot();profiles=[]
  ...
  with torch.no_grad():
   for entry in protocol['schedule']:
    ...
    prof=torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.CUDA]) if entry['phase']=='profile' else None
    ...
    if prof is not None:
     tracefile=out/f'trace-{mode}.json';prof.export_chrome_trace(str(tracefile));trace=json.loads(tracefile.read_text());groups={};kernels={}
     for e in trace['traceEvents']:
      if e.get('cat')!='kernel' or e.get('ph')!='X':continue
      name=e['name'];low=name.lower();duration=e.get('dur',0.)
      group='pooling' if 'mean_pool' in low else ('routing_sorting' if any(t in low for t in ['topk','sort','cub::']) else ('attention' if 'flash' in low else ('matrix' if any(t in low for t in ['gemm','gemv']) else 'other')))
      groups[group]=groups.get(group,0.)+duration;z=kernels.setdefault(name,dict(name=name,group=group,calls=0,microseconds=0.));z['calls']+=1;z['microseconds']+=duration
     record=dict(mode=mode,grouped_cuda_kernel_microseconds=groups,cuda_kernel_microseconds_total=sum(groups.values()),kernels=sorted(kernels.values(),key=lambda x:x['microseconds'],reverse=True),trace_file=tracefile.name,trace_sha256=sha(tracefile),classification='Heuristic kernel-name categories; summed kernel duration is not critical-path wall time. Instrumented timings are not benchmark timings.')
     profiles.append(record);save(out/f'profile-{mode}.json',record)
    del output,h,logits
    if len(predictions)%12==0:event('timing_progress',completed=len(predictions),maximum=len(protocol['schedule']))
  assert len(predictions)==6
  result=dict(status='complete',job=job,identity=identity,environment=environment,started_utc=started,finished_utc=utc(),seconds=time.perf_counter()-tick,step=step,optimizer_updates=0,task_predictions=len(predictions),calibration_values=vals,calibration_max_abs_error=error,checkpoint_sha256=sha(checkpoint),eval_source_sha256=sha(Path(__file__)),predictions=predictions,gpu_before=gpu_before,gpu_after=gpu_snapshot(),profiles=profiles)
 except Exception:
