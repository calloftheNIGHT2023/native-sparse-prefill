...
def main():
    a=R/'exports/midpoint64-evidence-v1.tar.gz';proof=load(a.with_suffix('.json'));assert sha(a)==proof['sha256'] and a.stat().st_size==proof['bytes']
    dest=R/'results/cloud-midpoint64-evidence-v1';dest.mkdir(exist_ok=True)
    ...
    pp=dest/'provenance/midpoint64-protocol-v1.json';assert sha(pp)==sha(R/'provenance/midpoint64-protocol-v1.json');p=load(pp)
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(dest/n)==h,n
    stage=dest/'results/midpoint64-stage-v1';control=load(stage/'result.json')
    ...
    for j,run in zip(p['jobs'],control['jobs']):
        ...
        assert v['eval_source_sha256']==sha(dest/'scripts/eval_midpoint64_v1.py') and sha(d/'source.py')==v['eval_source_sha256']
        ...
        train=load(mirror/j['training_result']);assert v['identity']==train['identity'] and {n:z for n,z in v['environment'].items() if n!='gpu'}=={n:z for n,z in train['environment'].items() if n!='gpu'}
    ...
    out=R/'results/midpoint64-audit-v1';out.mkdir(exist_ok=True)
