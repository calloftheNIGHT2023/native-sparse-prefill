...
def add_question_blocks(ids,start,target,reservoir,mode,block):
 n,h,k=ids.shape;extra=len(target);tail=ids[start:];assert bool((tail>=0).all())
 assert set(target).isdisjoint(reservoir) and len(set(reservoir))==len(reservoir)
 pool=target+reservoir if mode=='add_target' else reservoir
 assert max(pool)<start//block
 pool=torch.tensor(pool,device=ids.device,dtype=torch.int32)[None,None,:].expand(n-start,h,-1)
 occupied=(pool[...,None]==tail[:,:,None,:]).any(-1)
 ranks=torch.arange(pool.shape[-1],device=ids.device)[None,None,:].expand_as(pool)
 picked=torch.argsort(torch.where(occupied,ranks+pool.shape[-1],ranks),dim=-1)[...,:extra]
 added=torch.gather(pool,-1,picked);newtail=torch.cat([tail,added],dim=-1)
 assert bool((newtail.sort(-1).values[...,1:]!=newtail.sort(-1).values[...,:-1]).all())
 if mode=='add_target':assert bool((newtail[...,None]==torch.tensor(target,device=ids.device)[None,None,None,:]).any(-2).all())
 result=torch.nn.functional.pad(ids,(0,extra),value=-1);result[start:]=newtail
 assert torch.equal(result[:,:,:k],ids) and bool(((result>=0).sum(-1)[start:]==k+extra).all())
 assert bool((result<=torch.arange(n,device=ids.device)[:,None,None]//block).all())
 return result

def routed_attention(q,k,v,scale,mode='native',start=None,forced=None,target=None,telemetry=None,return_ids=False,sham_candidates=None):
 ...
 if mode=='native':
  ...
  if mode in ['add_target','add_sham']:ids=add_question_blocks(ids,start,target,sham_candidates,mode,block)
  elif mode=='forced':ids=replace_question_blocks(ids,start,forced,block)
