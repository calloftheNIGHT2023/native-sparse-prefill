...
def main():
    pp=R/'provenance/matched-restore-training-protocol-v0.json';p=json.loads(pp.read_text())
    for n,h in {**p['source_sha256'],**p['data_sha256']}.items():assert sha(R/n)==h,n
    assert not subprocess.check_output(['nvidia-smi','--query-compute-apps=pid','--format=csv,noheader'],text=True).strip()
    out=R/'results/matched-restore-training-stage-v0';out.mkdir(exist_ok=False);state=R/'logs/matched-restore-training-queue-v0.json';start=utc();tick=time.monotonic();jobs=[];replayed=set()
    ...
    try:
        for j in p['jobs']:
            ...
            cmd=[sys.executable,'-u',str(R/'scripts/train_matched_restore_v0.py'),'--job',j['name'],'--output',str(out/j['name'])]
    ...
    update_count=sum(json.loads(f.read_text())['optimizer_updates'] for f in out.glob('*/result.json'))
    if status=='complete':assert update_count==p['expected_optimizer_updates']
    gradient_count=sum(json.loads(f.read_text()).get('gradient_gate_passes',0) for f in out.glob('*/result.json'))
    if status=='complete':assert gradient_count==p['expected_gradient_passes']
    save(out/'result.json',dict(gradient_passes=gradient_count,status=status,started_utc=start,finished_utc=utc(),seconds=time.monotonic()-tick,jobs=jobs,optimizer_updates=update_count,task_predictions=count,protocol_sha256=sha(pp)))
    archive=R/'exports/matched-restore-training-evidence-v0.tar.gz';assert not archive.exists();entries=[]
    with tarfile.open(archive,'w:gz') as t:
        ...
        raw=json.dumps(dict(files=entries),indent=2).encode();m=tarfile.TarInfo('matched-restore-training-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
    proof=dict(sha256=sha(archive),bytes=archive.stat().st_size,files=len(entries));save(archive.with_suffix('.json'),proof);save(state,dict(status=status,utc=utc(),optimizer_updates=update_count,task_predictions=count,archive=proof))
