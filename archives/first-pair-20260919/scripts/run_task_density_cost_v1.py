...
out=R/'results/task-density-cost-stage-v1';out.mkdir(exist_ok=False)
start=utc();tick=time.monotonic();error=None
cmd=[sys.executable,'-u',str(R/'scripts/benchmark_task_density_cost_v1.py'),'--run-dir',str(R/'parents/dense-seed0'),'--output',str(out/'benchmark')]
...
files += [R/'scripts/benchmark_task_density_cost_v1.py',Path(__file__),R/'provenance/task-quality-density-cost-v1-protocol.json']
a=R/'exports/task-density-cost-evidence-v1.tar.gz';assert not a.exists();rows=[]
with tarfile.open(a,'w:gz') as t:
 ...
 raw=json.dumps(dict(files=rows,completed_job_directories=['results/task-density-cost-stage-v1/benchmark'] if code==0 else []),indent=2).encode();m=tarfile.TarInfo('amp-recovery-results-manifest.json');m.size=len(raw);t.addfile(m,io.BytesIO(raw))
