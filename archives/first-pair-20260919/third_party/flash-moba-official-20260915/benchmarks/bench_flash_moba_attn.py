...
for causal in causal_vals:
    for headdim in headdim_vals:
        for batch_size, seqlen in bs_seqlen_vals:
            ...
            moba_col_offsets, moba_col_nnz, moba_row_indices = flash_topk_varlen_func(
                q,
                k,
                cu_seqlens,
                cu_seqlens,
...
# export the time result to a csv file

# seqlen | batch_size | method | time_f | time_b | time_f_b
data = []
configs = sorted(list(set(c for c, m in time_f.keys())))
for method in methods:
    for config in configs:
        if (config, method) in time_f:
            seqlen = config[3]
            batch_size = config[2]
            time_f_val = time_f[config, method]
            time_b_val = time_b[config, method]
            time_f_b_val = time_f_b[config, method]
            data.append({
                'seqlen': seqlen,
                'batch_size': batch_size,
                'method': method,
                'time_f': f'{time_f_val*1000:.2f}',
                'time_b': f'{time_b_val*1000:.2f}',
                'time_f_b': f'{time_f_b_val*1000:.2f}'
            })

df = pd.DataFrame(data)
df.to_excel('flash_moba_attn_time.xlsx', index=False)
